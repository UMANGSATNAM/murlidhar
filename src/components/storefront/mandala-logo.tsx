...
export function MandalaLogo({ className = '', size = 48 }: { className?: string; size?: number }) {
  return (
    ...
    >
      ...
      <circle cx="50" cy="50" r="47" fill="none" stroke="#d4a017" strokeWidth="0.8" />

      {/* Mandala petals — 16 fold */}
      <g stroke="#d4a017" strokeWidth="0.6" fill="none" opacity="0.85">
        {Array.from({ length: 16 }).map((_, i) => {
          const a = (i * 360) / 16
          return (
            <g key={i} transform={`rotate(${a} 50 50)`}>
              <path d="M50 14 C54 24, 54 30, 50 36 C46 30, 46 24, 50 14 Z" />
              <circle cx="50" cy="20" r="0.8" fill="#d4a017" stroke="none" />
            </g>
          )
        })}
        {Array.from({ length: 16 }).map((_, i) => {
      ...
      <g fill="none" stroke="#d4a017" opacity="0.7">
      ...
      <g>
        {/* Flute body */}
        <rect x="48" y="38" width="4" height="24" rx="2" fill="#d4a017" />
        {/* Mouth hole */}
        ...
        {/* End caps */}
        <circle cx="50" cy="38" r="1.6" fill="#e6c068" />
        <circle cx="50" cy="62" r="1.6" fill="#e6c068" />
      </g>
